...
def test_edit_y_axis_label_with_no_plot(gui, mocker):

    plotter = Plotter_Function()
    message_box_mock = mocker.patch.object(QMessageBox, 'warning')

    # Call the function we want to test
    plotter.edit_y_axis_label(self_of_gui_object=gui)
    ...
    assert args[2] == "Please plot the function first before changing the y label"


# ( the user entered y label = "y" and then pressed ok )
def test_edit_y_axis_label_with_plot_exist_and_Data_Entered(gui, mocker):
    ...
    # Assert that y-label has been changed to the desired input
    assert plotter.ax.get_ylabel() == "y"


# ( the user pressed cancel )
def test_edit_y_axis_label_with_plot_exist_and_No_Data_Entered(gui, mocker):
    ...
    # Assert that y-label doesn't change
